...
```sh
make docker-build docker-push IMG=<some-registry>/empty:tag
```
...
```sh
make deploy IMG=<some-registry>/empty:tag
```
...
```sh
make build-installer IMG=<some-registry>/empty:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/empty/<tag or branch>/dist/install.yaml
```
